...
%>
		...
		<ul>
			<li>소속 마법사</li>
				<ul>
					...
					<li><a href="shopWizardDelete.jsp"><strong>삭제</strong></a></li>
				</ul>
	...
	<div id="container">
		<form method="get" action="shopWizardDelete.jsp">
		...
		<caption>내 소속 마법사 목록</caption>
	  		<thead>
   			...
   			<tbody>
   				...
   				while(rs.next()){
   					String wizardId = rs.getString("WizardID"); 
   					sql2 = "select * from wizards where WizardID ='"+wizardId+"';";
   					rs2 = stmt.executeQuery(sql2);

   				while(rs2.next()){
   					wId = rs2.getString("WizardID");
   					wName = rs2.getString("Name");
   					wAge = rs2.getInt("Age");
   					wTribe = rs2.getString("Tribe");
   					wHometown = rs2.getString("Hometown");
   					wOccupation = rs2.getString("Occupation");
   					wClass = rs2.getInt("Class");
   					wAttribute = rs2.getString("Attribute");
   					wMana = rs2.getInt("Mana");
   					wBalance =  rs2.getInt("Balance");
				...
				<tr>
					...
					onclick="location.href='shopWizardDeleteOK.jsp?wId=<%=wId%>'" >
					</td>
				</tr>
				<%
				}//while(rs)
   					...
   					if(searchRule.equals("이름")){
   	   				...
   	   				while(rs.next()){
   	   					String wizardId = rs.getString("WizardID"); 
   	   					sql2 = "select * from wizards where WizardID ='"+wizardId+"' and Name like '%"+searchStr+"%';";
   	   					rs2 = stmt.executeQuery(sql2);
   					...
   					<%
   	   				...
   	   				while(rs.next()){
   	   					String wizardId = rs.getString("WizardID"); 
   	   					sql2 = "select * from wizards where WizardID ='"+wizardId+"' and Tribe like '%"+searchStr+"%';";
   	   					rs2 = stmt.executeQuery(sql2);
   					...
   					<%
   	   				...
   	   				while(rs.next()){
   	   					String wizardId = rs.getString("WizardID"); 
   	   					sql2 = "select * from wizards where WizardID ='"+wizardId+"' and Class =" +searchStr+";";
   	   					rs2 = stmt.executeQuery(sql2);
   					...
   					<%
   	   				...
   	   				while(rs.next()){
   	   					String wizardId = rs.getString("WizardID"); 
   	   					sql2 = "select * from wizards where WizardID ='"+wizardId+"' and Attribute like '%"+searchStr+"%';";
   	   					rs2 = stmt.executeQuery(sql2);
